...
p2<-Vlntype("Gapdh")
p3<-Vlntype("Hprt")
pdf(file="H:/WRY/data/newRNA/BM/fig1/Vln_corgene_housekeep.pdf",width = 18,height=8)
plot_grid(p1,p2,p3,ncol = 1)
dev.off()

p1<-Vlntype("Ptprc")

pdf(file="H:/WRY/data/newRNA/BM/fig1/Vln_corgene_Ptprc.pdf",width = 18,height=2.5)
plot_grid(p1,ncol = 1)
dev.off()
#########################################################
n0<-readRDS(file = "H:/WRY/data/newRNA/SI/s00h/s00h_Seurat_cluster.rds")
###umap
pdf(file = "H:/WRY/data/newRNA/BM/fig1/SI_E_umap_n00h_label.pdf",width = 6,height = 5)
DimPlot(n0, reduction = "umap",label = TRUE, label.size = 4, pt.size = 0.6)
dev.off()
pdf(file = "H:/WRY/data/newRNA/BM/fig1/SI_E_umap_n00h_nolegend.pdf",width = 5,height = 5)
...
n0$new_ratio<-ratio[rownames(n0@meta.data),"new_ratio"]
pdf(file = "H:/WRY/data/newRNA/BM/fig1/SI_E_Vln_newratio_n00h1.pdf",width = 12,height = 6)
VlnPlot(n0, features = c("new_ratio"))+NoLegend()+xlab("")+
  theme(plot.title = element_blank())+
  theme(axis.title = element_text(size = 18))+
  theme(axis.text = element_text(size = 18,color="black"))+
  theme(axis.line = element_line(size=0.6, colour = 'black'))+
  theme(axis.ticks  = element_line(size=0.6, colour = 'black'))
dev.off()
pdf("H:/WRY/data/newRNA/BM/fig1/SI_E_new_ratio_n00h1.pdf", width = 5, height = 5)
FeaturePlot(n0, features = "new_ratio", reduction = "umap", label = FALSE,order = TRUE,pt.size = 0.6)+
  scale_color_gradientn(colors = color)
dev.off()
pdf("H:/WRY/data/newRNA/BM/fig1/SI_E_new_ratio_n00h1_nolegend.pdf", width = 5, height = 5)
FeaturePlot(n0, features = "new_ratio", reduction = "umap", label = FALSE,order = TRUE,pt.size = 0.6)+NoLegend()+
  scale_color_gradientn(colors = color)+
  theme(plot.title = element_blank())+
  theme(axis.title = element_text(size = 18))+
...
marker <- FindAllMarkers(new, only.pos = TRUE,min.pct = 0.1,logfc.threshold = 0.25)
marker <- marker[marker$p_val_adj < 0.05,]
write.csv(marker,file = "H:/WRY/data/newRNA/BM/fig1/SI_E_marker_new_n00h.csv")
top10 <- marker %>% group_by(cluster) %>% top_n(n = 10, wt = avg_log2FC)
...
dev.off()
####################################
n0<-readRDS(file = "H:/WRY/data/newRNA/SI/s00h/s00h_Seurat_cluster_Immune.rds")
###umap
pdf(file = "H:/WRY/data/newRNA/BM/fig1/SI_Immune_umap_n00h_label.pdf",width = 6,height = 5)
DimPlot(n0, reduction = "umap",label = TRUE, label.size = 4, pt.size = 0.6)
dev.off()
pdf(file = "H:/WRY/data/newRNA/BM/fig1/SI_Immune_umap_n00h_nolegend.pdf",width = 5,height = 5)
DimPlot(n0, reduction = "umap",label = FALSE, label.size = 4, pt.size = 0.6)+NoLegend()+
  theme(axis.title = element_text(size = 18))+
  theme(axis.text = element_text(size = 18,color="black"))+
  theme(axis.line = element_line(size=0.6, colour = 'black'))+
...
ratio<-read.csv(file = "H:/WRY/data/newRNA/SI/ntr/new_ratio_qc.csv",row.names = 1)
n0$new_ratio<-ratio[rownames(n0@meta.data),"new_ratio"]
pdf(file = "H:/WRY/data/newRNA/BM/fig1/SI_Immune_Vln_newratio_n00h1.pdf",width = 12,height = 6)
VlnPlot(n0, features = c("new_ratio"))+NoLegend()+xlab("")+
  theme(plot.title = element_blank())+
  theme(axis.title = element_text(size = 18))+
  theme(axis.text = element_text(size = 18,color="black"))+
  ...
  theme(axis.ticks  = element_line(size=0.6, colour = 'black'))
dev.off()
pdf("H:/WRY/data/newRNA/BM/fig1/SI_Immune_new_ratio_n00h1.pdf", width = 6, height = 5)
FeaturePlot(n0, features = "new_ratio", reduction = "umap", label = FALSE,order = TRUE,pt.size = 0.6)+
  scale_color_gradientn(colors = color)
dev.off()
pdf("H:/WRY/data/newRNA/BM/fig1/SI_Immune_new_ratio_n00h1_nolegend.pdf", width = 5, height = 5)
FeaturePlot(n0, features = "new_ratio", reduction = "umap", label = FALSE,order = TRUE,pt.size = 0.6)+NoLegend()+
  scale_color_gradientn(colors = color)+
  theme(plot.title = element_blank())+
  theme(axis.title = element_text(size = 18))+
...
marker <- FindAllMarkers(new, only.pos = TRUE,min.pct = 0.1,logfc.threshold = 0.25)
marker <- marker[marker$p_val_adj < 0.05,]
write.csv(marker,file = "H:/WRY/data/newRNA/BM/fig1/SI_Immune_marker_new_n00h.csv")
top10 <- marker %>% group_by(cluster) %>% top_n(n = 10, wt = avg_log2FC)
write.csv(top10,file = "H:/WRY/data/newRNA/BM/fig1/SI_Immune_marker_new_top10_n00h.csv")
new <- FindVariableFeatures(new)
new <- ScaleData(new,features = top10$gene)
data <- subset(new, downsample = 300)
pdf("H:/WRY/data/newRNA/BM/fig1/SI_Immune_Heatmap_new_top10.pdf", width = 12, height = 20)
DoHeatmap(data, features = top10$gene, size = 3, angle = 90, label = TRUE)+
  scale_fill_gradientn(colors = brewer.pal(n = 9, name = "Reds"))
dev.off()
pdf("H:/WRY/data/newRNA/BM/fig1/SI_Immune_Heatmap_new_top10_nolegend.pdf", width = 12, height = 8)
DoHeatmap(data, features = top10$gene, size = 3, angle = 90, label = FALSE)+NoLegend()+
  scale_fill_gradientn(colors = brewer.pal(n = 9, name = "Reds"))
dev.off()
pdf("H:/WRY/data/newRNA/BM/fig1/SI_Immune_Heatmap_new_top10_nolegend1.pdf", width = 12, height = 8)
DoHeatmap(data, features = top10$gene, size = 3, angle = 90, label = FALSE)+NoLegend()+
  scale_fill_gradientn(colors = rev(brewer.pal(n = 10, name = "RdBu")))
dev.off()
#############GO ntr
...
color<- c(blue='#4d85bd', grey = 'grey', yellow ='#f7903d',green="#59a95a",red="red")
data[data$NAME == "Ptprc","color"] <- "red"
data[data$NAME == "S100a9","color"] <- "red"
data[data$NAME == "Ccr2","color"] <- "red"
data[data$NAME == "Mki67","color"] <- "red"
data[data$NAME == "Camp","color"] <- "red"
data[data$NAME == "Rpl13","color"] <- "red"
data[data$NAME == "Rpl29","color"] <- "red"
pdf(file="H:/WRY/data/newRNA/BM/fig1/half_alpha_point_label.pdf",width=6,height=5)
ggplot(data,aes(log(half_life,base = 2),log(alpha,base = 2),color=color))+geom_point()+
  theme_classic()+
  scale_color_manual(values=color)+
  geom_hline(yintercept = log(a_cutoff,base = 2), lty=4,col='grey',lwd=0.6)+
